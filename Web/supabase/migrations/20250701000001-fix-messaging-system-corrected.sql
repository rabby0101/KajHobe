-- Fix messaging system for job applications and negotiations (Corrected Version)

-- First, check if conversations and messages tables exist from previous migrations
-- If not, we'll use the existing chat_messages table structure
...
CREATE POLICY IF NOT EXISTS "Users can insert conversations" 
  ON public.conversations 
  FOR INSERT 
  WITH CHECK (auth.uid() = client_id OR auth.uid() = provider_id);

CREATE POLICY IF NOT EXISTS "Users can update conversations they are part of" 
  ON public.conversations 
  FOR UPDATE 
  USING (auth.uid() = client_id OR auth.uid() = provider_id);
...
CREATE POLICY IF NOT EXISTS "Users can view offers in their conversations" 
  ON public.offers 
  FOR SELECT 
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can create offers in their conversations" 
  ON public.offers 
  FOR INSERT 
  WITH CHECK (
...
-- Only job owners can accept offers
CREATE POLICY IF NOT EXISTS "Job owners can update offer status" 
  ON public.offers 
  FOR UPDATE 
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Anyone can view open jobs" ON public.jobs
